...
class GraphConvolution:
    ...
    def __init__(self,
        ...
        self._groups = 2
        self._split_dim = self._hidden_dim // self._groups
        self._dropout = dropout
        self._norm = norm
        self._activation = activation
        self._weight_list = []
    ...
    def __call__(self, adj, inputs, seq_len):
        ...
        # split input with the number of heads
        input_list = mx.sym.split(inputs, axis=2, num_outputs=self._heads)
        outputs = input_list[0]
        ...
        for i in range(self._heads):
            ...
            if i < self._heads - 1:
                cache_list.append(input_list[i+1])
    ...
    def _convolve(self, adj, inputs, i, seq_len):
        ...
        for j in range(self._direction_num):
            ...
            inputs = mx.sym.reshape(inputs, shape=(-1, seq_len * self._groups, self._split_dim * (i * 2 + 1)))
            output = mx.sym.dot(inputs, weight)
            output = mx.sym.broadcast_add(output, bias)
            output = mx.sym.reshape(output, shape=(-1, seq_len, self._hidden_dim))
        ...
        if self._norm:
            ...
            final_outputs = mx.sym.broadcast_div(outputs, norm_factor)

        final_output = mx.sym.Activation(final_outputs, act_type=self._activation)
        return final_output
